...
const ExternalSNSConnector: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/ex-search-profiles?query=${encodeURIComponent(searchInput)}`,{
        method: 'GET',
        headers: {
  ...
  return (
    <div className="p-5 md:p-6 lg:p-8 responsive-container">
      <div className="mb-6 lg:mb-8 max-w-4xl mx-auto">
        <h2 className="text-lg md:text-xl font-semibold mb-3">숨은 전문가 찾기</h2>
      ...
      <div className="mb-6 max-w-4xl mx-auto">
          ...
          {searchResults.map((result, index) => (
              ...
              <div className="flex flex-col gap-2">
                  ...
                  className="flex items-center gap-2 font-semibold text-lg md:text-xl text-blue-600 hover:underline"
                  >
                  {result.title}
                  <ExternalLink size={18} strokeWidth={2} />
                  </a>
                  <p className="text-sm md:text-base text-gray-600">{result.snippet}</p>
              </div>
              </motion.div>
          ))}
          </motion.div>
...
export default ExternalSNSConnector;
